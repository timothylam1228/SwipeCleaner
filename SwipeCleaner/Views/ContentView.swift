...
struct ContentView: View {
    ...
    var body: some View {
            ...
            .toolbar {
                ToolbarItem(placement: .topBarLeading) {
                    Button("Undo", systemImage: "arrow.uturn.backward") {
                        library.undoLastDecision()
                    }
                    .disabled(library.lastHistoryEntry == nil)
                }
                ToolbarItemGroup(placement: .topBarTrailing) {
                    Button { showSimilarPhotos = true } label: {
                    ...
                    Button { showReview = true } label: {
                        Label(
                            "Review \(library.deletionQueue.count)",
                            systemImage: "trash"
                        )
                    }
                }
        ...
        .onChange(of: scenePhase) { _, phase in
            if phase == .active {
                Task { await library.refreshAuthorizationAndPhotos() }
            } else {
        ...
        .sheet(isPresented: $showDateRange) {
            DateRangePickerView(filter: library.dateFilter) {
                library.setDateFilter($0)
            }
        }
        .sheet(isPresented: $showSimilarPhotos) {
            SimilarPhotosView().environmentObject(library)
        }
        .fullScreenCover(
            isPresented: Binding(
                get: { detailAsset != nil },
                set: { if !$0 { detailAsset = nil } }
            )
        ) {
            if let detailAsset {
                PhotoDetailView(
    ...
    private var content: some View {
        ...
        case .empty:
            ContentUnavailableView {
                Label(
                    library.hasPhotosOutsideFilter
                        ? "No Photos in This Range" : "No Photos",
                    systemImage: "photo.on.rectangle.angled"
                )
            } description: {
                Text(
                    library.hasPhotosOutsideFilter
                        ? "Choose another time range to continue reviewing."
                        : "There are no accessible photos in your library."
                )
            } actions: {
                if library.hasPhotosOutsideFilter {
                    Button("Choose Time Range") {
                        showDateRange = true
                    }
    ...
    private var permissionIntro: some View {
        VStack(spacing: 20) {
            ...
            Text(
                "SwipeCleaner needs photo access so you can review and safely queue unwanted photos. Nothing is deleted without final confirmation."
            )
    ...
    private var deniedView: some View {
        ContentUnavailableView {
            ...
            Text(
                "Allow read and write access in Settings to review and delete photos."
            )
        } actions: {
    ...
    private func review(_ asset: PHAsset) -> some View {
        VStack(spacing: 12) {
            if library.isLimited {
                HStack {
                    ...
                    Button("Manage Access") {
                        library.presentLimitedLibraryPicker()
                    }
                    .font(.caption.bold())
                }
                .padding(10)
            ...
            HStack {
                Label(
                    "\(library.remainingCount) remaining",
                    systemImage: "photo"
                )
                Spacer()
                Label(
                    "\(library.deletionQueue.count) queued",
                    systemImage: "trash"
                )
            }
            ...
            ZStack {
                if let nextAsset = library.nextAsset {
                    PhotoImageView(
                        ...
                        targetSize: library.cardTargetSize
                    )
                    .clipShape(
                ...
                SwipeCardView(
                    ...
                    imageCache: library.imageCache,
                    targetSize: library.cardTargetSize,
                    onDecision: { library.decide($0) },
                    onDetails: { detailAsset = asset }
                )
    ...
    private var finishedView: some View {
        ContentUnavailableView {
            ...
            Text(
                library.deletionQueue.isEmpty
                    ? "You kept every photo."
                    : "Review the queued photos before deleting anything."
            )
        } actions: {
            if !library.deletionQueue.isEmpty {
                Button("Review \(library.deletionQueue.count) Photos") {
                    showReview = true
                }
                .buttonStyle(.borderedProminent)
                .tint(.red)
            }
            Button("Undo Last Swipe") {
                library.undoLastDecision()
            }
            .disabled(library.lastHistoryEntry == nil)
        }
