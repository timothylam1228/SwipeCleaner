...
struct SwipeCardView: View {
    ...
    var body: some View {
        GeometryReader { proxy in
            ...
            .gesture(DragGesture()
                ...
                .onEnded { value in
                    if value.translation.width < -threshold { animateOut(.delete, width: proxy.size.width) }
                    else if value.translation.width > threshold { animateOut(.keep, width: proxy.size.width) }
                    else { withAnimation(.spring(response: 0.35, dampingFraction: 0.72)) { offset = .zero } }
                })
        }
    ...
    private func indicator(_ text: String, color: Color, opacity: CGFloat) -> some View {
        Text(text).font(.title2.bold()).foregroundStyle(color).padding(10)
